...
class Card(Model):
    ...
    assigned_user_id = ForeignKey('User', on_delete=CASCADE, null=True, blank=True)
    delete_reason_id = ForeignKey('DeleteReason', on_delete=CASCADE, null=True, blank=True)
    column_id = ForeignKey('Column', on_delete=CASCADE)
    ...
    size = FloatField(null=True, blank=True)
    number = IntegerField()
